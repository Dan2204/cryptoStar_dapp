...
it('can add the star name and star symbol properly', async () => {
  let instance = await StarNotary.deployed();
  // CREATE A STAR AS PER PROJECT INSTRUCTIONS, BUT NOT REQUIRED TO TEST NAME AND SYMBOL //
  let user = accounts[1];
  let starId = 6;
  await instance.createStar('awesome star', starId, { from: user });
  let name = await instance.tokenName.call();
  let symbol = await instance.symbol.call();
  assert.equal(name, 'Star Notary Tokens');
...
it('lets 2 users exchange stars', async () => {
  ...
  // SEND PERMISSION FROM USER 2 TO ALLOW USER 1 TO MAKE SWAP //
...
it('lookUptokenIdToStarInfo test', async () => {
  ...
  let user1 = accounts[1];
  let starId = 10;
  await instance.createStar('Brand New Star', starId, { from: user1 });
  let starName = await instance.lookUptokenIdToStarInfo(starId);
  assert.equal(starName, 'Brand New Star');
  // 1. create a Star with different tokenId
